...
class Real_coded_GA : public EvolutionSearch {
public:
	Real_coded_GA(int populationSIze, float mutationProbability, float crossOverProbability, int overlapNum) :
...
private:
	...
	float To_Continuous_Value(vector<bool>& sample) {
		float arr[10] = { 2, 1.5, 1.25, 1, 0.75, 0.5, 0.2, 0.15, 0.1, 0.05 };
		float sum = 0;
		for (int i = 0; i < 10; i++) {
			if (sample[i] == true)
				sum += arr[i];
	...
	vector<float> B(vector<bool>& sample) {
		vector<float> x_list;
		for (int i = 0; i < 10; i++) {
			vector<bool> a;
			for (int j = 0; j < 10; j++) {
				a.push_back(sample[10 * i + j]);
			}
			x_list.push_back(To_Continuous_Value(a));
		}
		return x_list;
	}
	float A(vector<float> x) {
		float pow_sum = 0;
		for (int i = 0; i < 10; i++) {
			pow_sum += (x[i] * x[i]);
		}

		float a = sin(sqrt(pow_sum)) * sin(sqrt(pow_sum));
		float b = 1 + 0.001 * (pow(pow_sum, 2));
		return a / b;
	}
...
int main(void) {
	...
	srand(seed);
	Real_coded_GA evolution = Real_coded_GA(100, 0.01, 0.5, 20);
